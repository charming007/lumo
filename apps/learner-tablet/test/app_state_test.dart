...
void main() {
  group('LumoAppState learner assignment flow', () {
    ...
    test(
      'bootstrap keeps subject-shell modules live without forcing bundle hydration',
      () async {
        final apiClient = _SubjectShellBootstrapApiClient();
        final state = LumoAppState(
          apiClient: apiClient,
          includeSeedDemoContent: false,
        );
        ...
        expect(state.usingFallbackData, isFalse);
        expect(state.deploymentBlockerReason, isNull);
        expect(apiClient.requestedBundleModuleIds, isEmpty);
