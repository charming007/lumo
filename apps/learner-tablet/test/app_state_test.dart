...
void main() {
  group('LumoAppState learner assignment flow', () {
    ...
    test(
      'invalid production backend wiring still requires a hard deployment blocker even with a trusted offline snapshot',
      () {
        final apiClient = LumoApiClient(baseUrl: 'http://localhost:4000');
        final state = LumoAppState(
          includeSeedDemoContent: false,
          apiClient: apiClient,
        );
        ...
        state.usingFallbackData = true;
        state.deploymentBlockerReason =
            apiClient.invalidProductionBaseUrlReason;

        expect(state.hasUsableOfflineSnapshot, isTrue);
        expect(state.hasInvalidProductionBaseUrl, isTrue);
