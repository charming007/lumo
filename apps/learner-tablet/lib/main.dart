...
String _resolvedSubjectTitleForModule({
  ...
  if (lessonBackedSubject.isNotEmpty) {
    return lessonBackedSubject;
  }

  final learnerScopedSubject = learner == null
      ? ''
      : state
          .lessonsForLearner(learner)
          .map((lesson) => lesson.subject.trim())
          .firstWhere((subject) => subject.isNotEmpty, orElse: () => '');
  if (learnerScopedSubject.isNotEmpty) {
    return learnerScopedSubject;
  }
...
String resolveCapturedTranscriptForReview({
  ...
  final primaryTranscript =
      latestFinalTranscript.trim().isNotEmpty
...
class _LessonSessionPageState extends State<LessonSessionPage>
  ...
  Widget _buildDeviceDiagnosticsPanel() {
    ...
    final diagnosticCallouts = <String>[
      ...
      if (webSpeechRuntime != null && !webSpeechRuntime.isSpeechRecognitionExposed)
