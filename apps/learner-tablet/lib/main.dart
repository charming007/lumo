...
List<LearnerSubjectCardModel> buildLearnerSubjectCards({
  ...
  for (final lesson in lessonPool) {
    final subjectTitle = lesson.subject.trim().isNotEmpty
        ? lesson.subject.trim()
        : lesson.moduleId.trim().isNotEmpty
        ? lesson.moduleId.trim()
        : 'Learning';
    final subjectKey = _normalizeSubjectKey(subjectTitle);
    final module =
        state.modules.where((item) => item.id == lesson.moduleId).firstOrNull ??
        state.modules
            .where((item) => _normalizeSubjectKey(item.title) == subjectKey)
            .firstOrNull ??
        resolveLessonModule(state: state, lesson: lesson);
    addCard(subjectKey: subjectKey, subjectTitle: subjectTitle, module: module);
  }
  ...
  for (final module in state.modules) {
    final subjectTitle = module.title.trim().isNotEmpty
        ...
        : 'Learning';
    addCard(
      subjectKey: _normalizeSubjectKey(subjectTitle),
...
class SubjectModulesPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final nextAssignedLesson = state.nextAssignedLessonForLearner(
      selectedLearner,
    );
...
class _LessonLaunchSetupPageState extends State<LessonLaunchSetupPage> {
  ...
  LearnerProfile? _preferredLaunchLearner() {
    ...
    if (currentLearner != null &&
        widget.state.learners.any(
          (learner) => learner.id == currentLearner.id,
        )) {
    ...
    if (suggestedLearner != null &&
        widget.state.learners.any(
          (learner) => learner.id == suggestedLearner.id,
        )) {
