...
export function UpdateModuleFormClient({ modules }: { modules: CurriculumModule[] }) {
  ...
  return (
    <form action={updateModuleAction} style={cardStyle}>
      ...
      <FieldLabel>Status<select name="status" value={status} onChange={(event) => setStatus(event.target.value)} style={inputStyle}><option value="draft">Draft</option><option value="review">In review</option><option value="approved">Approved</option><option value="published">Published</option><option value="active">Active</option></select></FieldLabel>
